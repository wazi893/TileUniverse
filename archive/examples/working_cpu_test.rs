...
fn main() {
    ...
    println!("  Mux = 0x{:02X} (ROM[{}] = 0x{:02X})", mux & 0xFF, selector & 7, (rom_packed >> ((selector & 7) as u32 * 8)) & 0xFF);
    ...
    for cycle in 0..12 {
        ...
        let sel_str = if selector == u64::MAX { "MAX".to_string() } else { selector.to_string() };
        let match_str = if (selector & 7) == (pc & 7) { "" } else { " MISMATCH!" };

        println!(
            "  Cycle {:2}: clk={} PC={} sel={}{} Mux=0x{:02X}(exp 0x{:02X}) IR=0x{:02X} ({} deltas){}",
            cycle, clock, pc, sel_str, match_str, mux & 0xFF, expected, ir & 0xFF, stats.total_deltas,
            if !stats.converged { " [NOT CONVERGED]" } else { "" }
        );
    }
