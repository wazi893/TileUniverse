...
use engine::tile_cpu::{TileCpuBuilder, TileCpu, MinimalCpu};
...
impl RealCpuCluster {
    ...
    pub fn build(sim: &mut Simulation, programs: &[&[u8]; 4]) -> Self {
        ...
        for grid_y in 0..2 {
            for grid_x in 0..2 {
                ...
                let origin = (
                    20 + grid_x * cpu_spacing,
                    20 + grid_y * cpu_spacing,
                );
    ...
    fn wire_connections(sim: &mut Simulation, nodes: &[ClusterNode], _spacing: usize) {
        ...
        for (from_node, from_dir, to_node, to_dir) in pairs {
            ...
            if from_y == to_y {
                let (start_x, end_x) = if from_x < to_x { (from_x, to_x) } else { (to_x, from_x) };
                for x in (start_x + 1)..end_x {
                    sim.set_tile(x, from_y, TileType::WireH);
                }
            } else if from_x == to_x {
                let (start_y, end_y) = if from_y < to_y { (from_y, to_y) } else { (to_y, from_y) };
    ...
    pub fn tick_cpu(&mut self, sim: &mut Simulation, cpu_id: usize) -> engine::simulation::TimingStats {
                ...
                TileType::WireH | TileType::WireV | TileType::Wire => {
                    ...
                    if val != 0 {
                        ...
                        if old_val != val {
                            ...
                            if x > 0 { sim.dirty.mark_dirty(y * width + (x - 1)); }
                            if x + 1 < width { sim.dirty.mark_dirty(y * width + (x + 1)); }
                            if y > 0 { sim.dirty.mark_dirty((y - 1) * width + x); }
                            if y + 1 < height { sim.dirty.mark_dirty((y + 1) * width + x); }
                        }
                    }
                }
                _ => {}
    ...
    pub fn dump_state(&self, sim: &Simulation) -> String {
        ...
        s.push_str(&format!("=== Real CPU Cluster State (tick {}) ===\n", self.total_ticks));
        ...
        for node in &self.nodes {
            ...
            s.push_str(&format!(
                ...
                node.id, node.grid_pos.0, node.grid_pos.1,
                pc, r0, r1, r2, r3, node.cpu.tile_count
            ));
...
fn main() {
    ...
    println!("Total CPU tiles: {} ({} per CPU)", total_cpu_tiles, total_cpu_tiles / 4);
    println!("Grid size: {}x{} = {} tiles", sim.width(), sim.height(), sim.width() * sim.height());
    println!("CPU tile density: {:.2}%\n", 100.0 * total_cpu_tiles as f64 / (sim.width() * sim.height()) as f64);
    ...
    for cycle in 0..3 {
        ...
        for node in &cluster.nodes {
            ...
            println!("  CPU[{}]: PC {:02X}->{:02X}, R0 {}->{}, deltas={}, conv={}",
                node.id, pc_before, pc_after, r0_before, r0_after,
                stats.total_deltas, stats.converged);
        }
        println!();
    }
    ...
    println!("1000 propagations: {:?} ({:.0} prop/sec)", elapsed, 1000.0 / elapsed.as_secs_f64());
    ...
    let elapsed = start.elapsed();
    println!("1000 send+propagate: {:?} ({:.0} cycles/sec)", elapsed, 1000.0 / elapsed.as_secs_f64());

    // Show dirty tile stats
    let dirty = cluster.dirty_count(&sim);
    println!("Current dirty tiles: {} (sparse eval only touches these)\n", dirty);
    ...
    println!("  - Each CPU built from {} actual tiles", total_cpu_tiles / 4);
