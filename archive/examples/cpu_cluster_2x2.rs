...
impl CpuCluster {
    ...
    pub fn build(sim: &mut Simulation) -> Self {
        ...
        for grid_y in 0..2 {
            for grid_x in 0..2 {
                ...
                let origin = (
                    30 + grid_x * cpu_spacing,
                    30 + grid_y * cpu_spacing,
                );
    ...
    pub fn dump_state(&self, sim: &Simulation) -> String {
        ...
        s.push_str(&format!("=== CPU Cluster State (tick {}) ===\n", self.total_ticks));
...
fn main() {
    ...
    println!("Before propagation: CPU[1] West mailbox_in = {}", cluster.recv(&sim, 1, Direction::West));

    // Propagate through wires
    cluster.propagate(&mut sim);
    println!("After 1 propagate: CPU[1] West mailbox_in = {}", cluster.recv(&sim, 1, Direction::West));
    ...
    for _ in 0..10 { cluster.propagate(&mut sim); }
