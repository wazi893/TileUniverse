...
use engine::tile_cpu::minimal::{build_add_test, build_pc_ir, build_pc_only};
...
fn main() {
    ...
    println!(
        ...
        init.total_deltas, init.critical_path_deltas
    );
    ...
    for i in 0..5 {
        ...
        println!(
            ...
            i, stats.total_deltas, pc, ir, stats.converged
        );
        if !stats.converged {
            println!("  FAIL: Non-convergence!");
            break;
    ...
    if !init.converged {
        println!(
            "  WARNING: Did not converge on init ({} deltas)",
            init.total_deltas
        );
