//! Hybrid Brain Experiment: Classical NN vs QUBO vs Hybrid (NN+QUBO)
//!
//! Tests the hypothesis that hybrid NN+QUBO approach can outperform
//! both pure classical NN and pure QUBO by combining:
//! - NN exploitation (good at smooth gradient landscapes)
//! - QUBO exploration (structured sampling when uncertain)
//!
//! Run with: cargo run --release --example hybrid_experiment
...
fn compute_stats(trials: &[TrialResult]) -> GroupStats {
    ...
    let auc_variance = aucs.iter().map(|a| (a - mean_auc).powi(2)).sum::<f64>() / n;
    let std_auc = auc_variance.sqrt();
...
fn main() {
    println!("============================================================");
    println!("  HYBRID BRAIN EXPERIMENT");
    println!("  Classical NN vs QUBO vs Hybrid (NN + QUBO Exploration)");
    println!("============================================================");
    println!();
    println!("Hypothesis: Hybrid approach outperforms both pure methods by:");
    println!("  - Using NN for exploitation (smooth gradients)");
    println!("  - Using QUBO for exploration (structured sampling)");
    println!();

    // Create config
    let mut config = ThreeWayConfig::default();
    config.replications = 30; // Statistical power
    ...
    println!(
        "| Brain Type                | Mean Pop | Std Pop  | Mean AUC   | Extinctions| MaxGen |"
    );
    println!(
        "+---------------------------+----------+----------+------------+------------+--------+"
    );
    ...
    let hybrid_stats = stats
        .iter()
        .find(|s| s.brain_type == BrainType::HybridExploration)
        .unwrap();
    ...
    let hybrid_vs_classical_pct = if classical_stats.mean_auc > 0.0 {
        ((hybrid_stats.mean_auc - classical_stats.mean_auc) / classical_stats.mean_auc) * 100.0
    } else {
        0.0
    };

    let hybrid_vs_qubo_pct = if qubo_stats.mean_auc > 0.0 {
        ((hybrid_stats.mean_auc - qubo_stats.mean_auc) / qubo_stats.mean_auc) * 100.0
    } else {
        0.0
    };
    ...
    println!();
    println!(
        "Hybrid vs Classical NN: {:+.1}% AUC",
        hybrid_vs_classical_pct
    );
    println!("Hybrid vs QUBO:         {:+.1}% AUC", hybrid_vs_qubo_pct);
    println!();

    if hybrid_vs_classical_pct > 5.0 && hybrid_vs_qubo_pct > 5.0 {
        println!(">>> HYPOTHESIS CONFIRMED: Hybrid outperforms both pure methods!");
    } else if hybrid_vs_classical_pct > 0.0 && hybrid_vs_qubo_pct > 0.0 {
        println!(">>> Hybrid shows improvement but not statistically significant");
    } else if hybrid_vs_classical_pct > hybrid_vs_qubo_pct {
        println!(">>> Hybrid is between Classical and QUBO performance");
    } else {
        println!(">>> Hypothesis NOT confirmed - need to tune hybrid parameters");
    }

    // Extinction analysis
    println!();
    println!("EXTINCTION ANALYSIS:");
    println!(
        "  Classical NN: {}/{} ({:.0}%)",
        classical_stats.extinctions,
        classical_stats.n_trials,
        (classical_stats.extinctions as f64 / classical_stats.n_trials as f64) * 100.0
    );
    println!(
        "  QUBO:         {}/{} ({:.0}%)",
        qubo_stats.extinctions,
        qubo_stats.n_trials,
        (qubo_stats.extinctions as f64 / qubo_stats.n_trials as f64) * 100.0
    );
    println!(
        "  Hybrid:       {}/{} ({:.0}%)",
        hybrid_stats.extinctions,
        hybrid_stats.n_trials,
        (hybrid_stats.extinctions as f64 / hybrid_stats.n_trials as f64) * 100.0
    );
}
