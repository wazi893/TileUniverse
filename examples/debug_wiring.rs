...
fn main() {
    ...
    for i in 0..5 {
        ...
        println!(
            "  Tick {}: {} deltas, converged={}",
            i, stats.total_deltas, stats.converged
        );
        if !stats.converged {
    ...
    sim3.dirty.mark_all_dirty(sim3.tile_count());
    let stats = sim3.tick_with_delays();
    println!(
        "Wire ring: {} deltas, converged={}",
    ...
    sim4.dirty.mark_all_dirty(sim4.tile_count());
    let stats = sim4.tick_with_delays();
    println!(
        "NOT oscillator: {} deltas, converged={}",
        stats.total_deltas, stats.converged
    );
    if !stats.converged {
