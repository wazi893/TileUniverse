...
fn main() {
    ...
    for &seed in seeds {
        ...
        // M11 cached-rate path
        {
            ...
            let cached_rates =
                CachedRates::load(&rates_path).unwrap_or_else(|e| panic!("M11: {rates_path}: {e}"));
        ...
        // M12 live LIF path
        {
            ...
            let model = match LiveSnnModel::load(&snn_path) {
                ...
                Err(e) => {
                    println!(
                        "  M12 live    SKIP — checkpoints/m10_seed{seed}.snn not loadable: {e}"
                    );
                    continue;
                }
            };
            ...
            let direct_correct = {
                ...
                for idx in 0..demo_count {
                    ...
                    let rates_f: Vec<f32> = hidden_counts
                        .iter()
                        .map(|&c| c as f32 / model.n_ticks as f32)
                        .collect();
                    if model.mlp.forward_cpu(&rates_f) == test_labels[idx] {
                        ok += 1;
                    }
                }
                ok
    ...
    for &mode in ["M11 cached", "M12 live"].iter() {
        ...
        let mean_acc: f32 = mode_rows
            .iter()
            .map(|r| 100.0 * r.v2_correct as f32 / demo_count as f32)
            .sum::<f32>()
            / mode_rows.len() as f32;
        let mean_var: f32 = mode_rows
            .iter()
            .map(|r| {
    ...
    println!(
        "Overall parity:  {}",
        if all_parity {
            "ALL MATCH (V2 == direct reference for every row)"
        } else {
